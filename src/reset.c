...
char *sprint_reset(RESET_DATA *pReset, short *num)
{
   ...
   case 'M':
      ...
      snprintf(buf, MAX_STRING_LENGTH, "%2d) %.2015s (%d) -> %.2015s Room: %d [%d]\r\n", *num, mobname, pReset->arg1,
   ...
   case 'O':
      ...
      if (!room)
         ...
         strncpy(roomname, room->name, MAX_STRING_LENGTH);
      snprintf(buf, MAX_STRING_LENGTH, "%2d) (object) %.2012s (%d) -> %.2012s Room: %d [%d]\r\n",
   ...
   case 'D':
      ...
      snprintf(buf, MAX_STRING_LENGTH, "%2d) %.1342s [%d] the %.1342s [%d] door %.1342s (%d)\r\n",
   ...
   case 'R':
      ...
      snprintf(
          buf,
          MAX_STRING_LENGTH,
          "%2d) Randomize exits 0 to %d -> %.4033s (%d)\r\n",
          *num,
          pReset->arg2,
          roomname,
          pReset->arg1);
