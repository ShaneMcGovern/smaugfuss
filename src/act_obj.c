...
void do_get(CHAR_DATA *ch, const char *argument)
{
   ...
   else
   {
      ...
      case ITEM_CORPSE_PC:
      {
         ...
         if (IS_OBJ_STAT(container, ITEM_CLANCORPSE) && !IS_NPC(ch) && !IS_IMMORTAL(ch) && container->action_desc[0] != '\0' && str_cmp(name, ch->name) && str_cmp(container->action_desc, ch->name))
         {
...
void do_put(CHAR_DATA *ch, const char *argument)
{
   ...
   else
   {
      ...
      for (obj = ch->first_carrying; obj; obj = obj_next)
      {
         ...
         if ((fAll || nifty_is_name(chk, obj->name)) && can_see_obj(ch, obj) && obj->wear_loc == WEAR_NONE && obj != container && can_drop_obj(ch, obj) && (container->item_type != ITEM_KEYRING || obj->item_type == ITEM_KEY) && (container->item_type != ITEM_QUIVER || obj->item_type == ITEM_PROJECTILE) && get_obj_weight(obj) + get_obj_weight(container) <= container->value[0])
         {
            if (number && (cnt + obj->count) > number)
               split_obj(obj, number - cnt);
            cnt += obj->count;
...
void wear_obj(CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace, short wear_bit)
{
   ...
   case ITEM_WEAR_SHIELD:
      if (get_eq_char(ch, WEAR_DUAL_WIELD) || (get_eq_char(ch, WEAR_WIELD) && get_eq_char(ch, WEAR_MISSILE_WIELD)) || (get_eq_char(ch, WEAR_WIELD) && get_eq_char(ch, WEAR_HOLD)))
      {
      ...
      if (auction->item != NULL)
      {
         ...
         case ITEM_QUIVER:
            ch_printf(ch, "%s appears to %s.\r\n", capitalize(obj->short_descr),
                      obj->value[0] < 76 ? "have a small capacity" : obj->value[0] < 150 ? "have a small to medium capacity"
...
void do_rolldie(CHAR_DATA *ch, const char *argument)
{
   ...
   /**
    * Hmmm, verb length. In English this would be "antidisestablishmentarianism" but
    * it Turkish "muvaffakiyetsizleştiricileştiriveremeyebileceklerimizdenmişsinizcesine"...
    * Settle on 32.
    *
    * OK, I won't make you search. It means:
    * "As if you are one of those whom we may not be able to easily make into a maker of unsuccessful ones"
    */
   snprintf(output_string, MAX_STRING_LENGTH, "You %.32s%.4059s", verb, roll_string);
   act(AT_GREEN, output_string, ch, NULL, NULL, TO_CHAR);

   snprintf(output_string, MAX_STRING_LENGTH, "$n %.32s%.4030s", verb, roll_string);
