...
void do_goto(CHAR_DATA *ch, const char *argument)
{
   ...
   if (!xIS_SET(ch->act, PLR_WIZINVIS))
      act(AT_IMMORT, "$n $T", ch, NULL,
          (ch->pcdata && ch->pcdata->bamfout[0] != '\0') ? ch->pcdata->bamfout : (char *)"leaves in a swirling mist.", TO_ROOM);
   ...
   if (!xIS_SET(ch->act, PLR_WIZINVIS))
      act(AT_IMMORT, "$n $T", ch, NULL,
          (ch->pcdata && ch->pcdata->bamfin[0] != '\0') ? ch->pcdata->bamfin : (char *)"appears in a swirling mist.", TO_ROOM);
...
void do_rdig(CHAR_DATA *ch, const char *argument)
{
   ...
   if (!xit)
   {
      ...
      snprintf(tmpcmd, MAX_INPUT_LENGTH, "bexit %.1006s %d", arg, vnum);
      do_redit(ch, tmpcmd);
   }
   else
   {
...
void do_redit(CHAR_DATA *ch, const char *argument)
{
   ...
   if (!str_cmp(arg, "bexit"))
   {
      ...
      // "southwest", "somewhere" &c is 9 chr max
      snprintf(tmpcmd, MAX_INPUT_LENGTH, "exit %.9s %.9s %.998s", arg2, arg3, argument);
      do_redit(ch, tmpcmd);
      if (numnotdir)
      ...
      if (vnum)
      {
         // rvnum is an (unsigned?) int from 1 - 4294967295
         snprintf(tmpcmd, MAX_INPUT_LENGTH, "%d redit exit %d %.10s %.979s", vnum, rev_dir[edir], rvnum, argument);
         do_at(ch, tmpcmd);
      }
      return;
   }
...
void fwrite_fuss_affect(FILE *fp, AFFECT_DATA *paf)
{
   if (paf->type < 0 || paf->type >= num_skills)
   {
      fprintf(fp, "Affect       %d %d %d %d %s\n",
              paf->type,
              paf->duration,
              ((paf->location == APPLY_WEAPONSPELL || paf->location == APPLY_WEARSPELL || paf->location == APPLY_REMOVESPELL || paf->location == APPLY_STRIPSN || paf->location == APPLY_RECURRINGSPELL) && IS_VALID_SN(paf->modifier))
...
void old_fold_area(AREA_DATA *tarea, char *filename, bool install)
{
   ...
   for (vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++)
   {
      ...
      if (install)
         xREMOVE_BIT(pMobIndex->act, ACT_PROTOTYPE);
      if (pMobIndex->perm_str != 13 || pMobIndex->perm_int != 13 || pMobIndex->perm_wis != 13 || pMobIndex->perm_dex != 13 || pMobIndex->perm_con != 13 || pMobIndex->perm_cha != 13 || pMobIndex->perm_lck != 13 || pMobIndex->hitroll != 0 || pMobIndex->damroll != 0 || pMobIndex->race != 0 || pMobIndex->Class != 3 || !xIS_EMPTY(pMobIndex->attacks) || !xIS_EMPTY(pMobIndex->defenses) || pMobIndex->height != 0 || pMobIndex->weight != 0 || pMobIndex->speaks != 0 || pMobIndex->speaking != 0 || pMobIndex->xflags != 0 || pMobIndex->numattacks != 0)
         complexmob = TRUE;
      else
         complexmob = FALSE;
      fprintf(fpout, "#%d\n", vnum);
   ...
   for (vnum = tarea->low_o_vnum; vnum <= tarea->hi_o_vnum; vnum++)
   {
      ...
      for (paf = pObjIndex->first_affect; paf; paf = paf->next)
         fprintf(fpout, "A\n%d %d\n", paf->location,
                 ((paf->location == APPLY_WEAPONSPELL || paf->location == APPLY_WEARSPELL || paf->location == APPLY_REMOVESPELL || paf->location == APPLY_STRIPSN || paf->location == APPLY_RECURRINGSPELL) && IS_VALID_SN(paf->modifier)) ? skill_table[paf->modifier]->slot : paf->modifier);
