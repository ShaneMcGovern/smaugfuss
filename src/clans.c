...
void do_induct(CHAR_DATA *ch, const char *argument)
{
   ...
   if ((ch->pcdata && ch->pcdata->bestowments && is_name("caninduct", ch->pcdata->bestowments)) || !str_cmp(ch->name, clan->deity) || !str_cmp(ch->name, clan->leader) || !str_cmp(ch->name, clan->number1) || !str_cmp(ch->name, clan->number2))
      ;
   else
   {
      send_to_char("Huh?\r\n", ch);
...
void do_outcast(CHAR_DATA *ch, const char *argument)
{
   ...
   if ((ch->pcdata && ch->pcdata->bestowments && is_name("canoutcast", ch->pcdata->bestowments)) || !str_cmp(ch->name, clan->deity) || !str_cmp(ch->name, clan->leader) || !str_cmp(ch->name, clan->number1) || !str_cmp(ch->name, clan->number2))
      ;
   else
   {
      send_to_char("Huh?\r\n", ch);
...
void do_showclan(CHAR_DATA *ch, const char *argument)
{
   ...
   ch_printf_color(ch, "\r\n&w%s    : &W%s\t\t&wBadge: %s\r\n&wFilename : &W%s\r\n&wMotto    : &W%s\r\n",
                   clan->clan_type == CLAN_ORDER ? "Order" : (clan->clan_type == CLAN_GUILD ? "Guild" : "Clan "),
