...
bool check_ability(CHAR_DATA *ch, char *command, char *argument)
{
   ...
   {
      mana =
          IS_NPC(ch) ? 0 : UMAX(skill_table[sn]->min_mana, 100 / (2 + ch->level - skill_table[sn]->race_level[ch->race]));
      blood = (mana / 2);
      if (IS_VAMPIRE(ch))
      {
...
void do_search(CHAR_DATA *ch, const char *argument)
{
   ...
   if (door != -1)
   {
      ...
      if ((pexit = get_exit(ch->in_room, door)) != NULL && IS_SET(pexit->exit_info, EX_SECRET) && IS_SET(pexit->exit_info, EX_xSEARCHABLE) && can_use_skill(ch, percent, gsn_search))
      {
...
void do_broach(CHAR_DATA *ch, const char *argument)
{
   ...
   {
      EXIT_DATA *pexit_rev;
      if (!IS_SET(pexit->exit_info, EX_CLOSED) || !IS_SET(pexit->exit_info, EX_LOCKED) || IS_SET(pexit->exit_info, EX_PICKPROOF) || can_use_skill(ch, number_percent(), gsn_broach))
      {
...
void do_style(CHAR_DATA *ch, const char *argument)
{
   ...
   if (arg[0] == '\0')
   {
      ch_printf_color(ch, "&wAdopt which fighting style?  (current:  %s&w)\r\n",
                      ch->style == STYLE_BERSERK ? "&Rberserk" : ch->style == STYLE_AGGRESSIVE ? "&Raggressive"
